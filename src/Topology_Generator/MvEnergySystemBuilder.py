...
from Topology_Generator.dataclasses import EnergySystemOutput, EsdlNetworkTopology
    ...
    def init_archetype_networks(self, archetypes_esdls_paths : dict) -> dict[int : List[List[EsdlNetworkTopology]]]:
        ...
        for archetype, esdl_paths in archetypes_esdls_paths.items():
            ...
            for esdl_path in esdl_paths:
                print(f"Parsing esdl file: {esdl_path}")
                ...
                print(f"Finished parsing esdl file: {esdl_path}")
        return ret_val
    ...
    def build_mv_energy_system(self, mv_network : EnergySystem) -> EnergySystemOutput:
        if len(mv_network.instance) == 1:
            ...
            for transfomer in transfomers:
                print("Next transformer")
                transfomer_point = Point(transfomer.geometry.lat, transfomer.geometry.lon)
                network_topology_pairs = self.lv_network_builder.extract_lv_networks_and_topologies_at_point(transfomer_point)
                if len(network_topology_pairs) > 0:
                    ...
                    total_amount_of_connections_lv_networks = sum([network_topology_pair.amount_of_connections for network_topology_pair in network_topology_pairs])
                    total_amount_of_length_lv_networks = sum([network_topology_pair.total_length for network_topology_pair in network_topology_pairs])
                    for i, network_collection in enumerate(network_collections):
                        total_disatance = 0
                        new_matches = []
                        topology_analyzer = TopologyAnalyzer(network_collection)
                        # for network_topology_pair in network_topology_pairs:
                        #     network_plotter = NetworkPlotter(1,2)
                        #     network_plotter.plot_network(network_topology_pair.network_lines, without_axis_numbers=True)
                        #     network_plotter.plot_network_topology(network_topology_pair.network_topology)
                        #     network_plotter.show_plot()
                        for network_topology_pair in network_topology_pairs:
                            lv_lines_to_vizualize.extend(network_topology_pair.network_lines)
                            network_distance, network_with_min_distance = topology_analyzer.find_best_matching_network(network_topology_pair)
                            total_disatance += network_distance
                            new_matches.append(network_with_min_distance)
                        if total_disatance <= min_total_distance:
                            print(f"New min total distance: {total_disatance} using collection {i}")
                    ...
                    print(f"Total amount of connections in found lv network: {total_amount_of_connections_lv_networks} total amount of connections in the best matches: {total_amount_of_connections_best_match}")
                    print(f"Total length in found lv network: {total_amount_of_length_lv_networks} total amount of length in the best matches: {total_amount_of_length_best_match}")
